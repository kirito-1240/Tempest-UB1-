...
async def basicgroup_handler(_, m: Message):
    ...
    if app.long(m) == 1:
        return await app.send_edit(
            m, f"Usage: `{app.PREFIX}bgroup mygroupname`", delme=4
        )
    elif app.long(m) > 1:
    ...
    try:
        if grpname:
            ...
            await app.send_edit(
                m, "No group name is provided.", text_type=["mono"], delme=4
            )
...
async def supergroup_handler(_, m: Message):
    ...
    if app.long(m) == 1:
        return await app.send_edit(
            m, f"`Usage: {app.PREFIX}sgroup mygroupname`", delme=4
        )
    elif app.long(m) > 1:
        grpname = m.text.split(None, 1)[1]
        about = ""
...
async def channel_handler(_, m: Message):
    ...
    if app.long(m) == 1:
        return await app.send_edit(
            m, f"Usage: `{app.PREFIX}channel [channel name]`", delme=4
        )
